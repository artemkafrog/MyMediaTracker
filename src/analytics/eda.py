...
class ExploratoryAnalyzer:
    ...
    def get_plot_rating_distribution(self, save: bool = True, show: bool = True) -> Path | None:
        ...
        plt.axvline(avg_value, color="red", linestyle='-', linewidth=2, label=f"avg value: {avg_value:.2f}")
        plt.axvline(median_value, color="orange", linestyle="--", linewidth=2, label=f"median value: {median_value:.2f}")
        plt.xlabel("Rating")
        plt.ylabel("Amount")
        ...
        if save:
            ...
            file_name = f"rating_distribution_{timestamp}.png"
    ...
    def get_plot_status_pie(self, save: bool = True, show: bool = True) -> Path | None:
        ...
        colors = {"WATCHED": "#2ecc71", "WATCHING": "#3498db", "PLANNED": "#f39c12", "ON_HOLD": "#e76f3c"}
        ...
        if save:
            timestamp = datetime.now().timestamp()
            file_name = f"status_pie_{timestamp}.png"
            file_path = self._figures_path / file_name 
            plt.savefig(file_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
    ...
    def get_plot_rating_by_status(self, save: bool = True, show: bool = True) -> Path | None:
        ...
        if save:
            timestamp = datetime.now().timestamp()
            file_name = f"rating_by_status_{timestamp}.png"
            file_path = self._figures_path / file_name
            plt.savefig(file_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
    ...
    def get_plot_releases_over_time(self, freq: str = 'Y', save: bool = True, show: bool = True) -> Path | None:
        ...
        plt.plot(releases_by_year.index, releases_by_year.values, marker='o', linewidth=2, markersize=8, label='Releases')
        
        if len(releases_by_year) >= 3:
            rolling_mean = releases_by_year.rolling(3, min_periods=1).mean()
            plt.plot(rolling_mean.index, rolling_mean.values, linestyle='--', color='red', linewidth=2, label='Rolling Mean (3 years)')
        ...
        if save:
            timestamp = datetime.now().timestamp()
            file_name = f"releases_over_time_{timestamp}.png"
            file_path = self._figures_path / file_name
            plt.savefig(file_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
    ...
    def get_plot_duration_distribution(self, save: bool = True, show: bool = True) -> Path | None:
        plt.figure(figsize=(10, 6))
        ...
        plt.axvline(median_value, color='red', linestyle='-', linewidth=2, label=f'Median: {median_value:.0f} min')
        ...
        if save:
            timestamp = datetime.now().timestamp()
            file_name = f"duration_distribution_{timestamp}.png"
            file_path = self._figures_path / file_name
            plt.savefig(file_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
    ...
    def get_plot_correlation_heatmap(self, save: bool = True, show: bool = True) -> Path | None:
    ...
    def get_plot_genre_popularity(self, save: bool = True, show: bool = True) -> Path | None:
        ...
        if save:
            timestamp = datetime.now().timestamp()
            file_name = f"genre_popularity_{timestamp}.png"
            file_path = self._figures_path / file_name
            plt.savefig(file_path, dpi=300, bbox_inches="tight")
        if show:
            plt.show()
