...
class ContentRecommender:
    ...
    def _build_feature_matrix(self) -> Tuple[pd.DataFrame, List[str]]:
        ...
        feature_cols = [col for col in df_copy.columns
                        if col.startswith(('genre_', 'type_')) or col.endswith('_normalized')]

        self._feature_matrix = df_copy[['id'] + feature_cols].set_index('id')
        self._feature_names = feature_cols
        return self._feature_matrix, self._feature_names

    def recommend_similar(self, item_id: int, top_n: int = 5,
                          exclude_statuses: List[str] = None) -> List[Tuple[int, str, float]]:
    ...
    def recommend_by_status(self, status: str, top_n: int = 5,
                            exclude_self: bool = True) -> List[Tuple[int, str, float]]:
        """Recommend items from a specific status group."""
        features, _ = self._build_feature_matrix()
...
class StatusPredictor:
    """Predicts whether an item will be watched using ML."""
    ...
    def predict_status(self, features: Dict[str, Any]) -> int:
        """Predict whether a new item will be watched."""
