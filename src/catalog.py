            ...
            if has_filters:
                ...
                if parsed_query.get("title"):
                    ...
                    if title_candidates:
                        ...
                        items = [self._items[id] for id in candidates if id in self._items]
                        if items:
                            try:
                ...
                search_title = parsed_query.get("title", query)
                return self._search_by_title(search_title)
    ...
    def search_all(self, query: str = "") -> list[MediaItem]:
        ...
        try:
            ...
            if parsed_query.get("genre"):
                ...
                for item in catalog:
                    try:
                        if any(genre in g.lower() for g in item.genres):
                            results.append(item)
                    except Exception:
                        continue
                return results
