...
class DatabaseManager:
    ...
    def _item_to_db_data(self, item: MediaItem) -> dict:
        """Convert MediaItem to database-ready dict."""
        return {
            'title': item.title,
    ...
    def add_item(self, item: MediaItem) -> int:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO media_items (title, media_type, status, rating, release_date,
                                         genres, description, authors, video_path, duration)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                data['title'], data['media_type'], data['status'], data['rating'],
                data['release_date'], data['genres'], data['description'],
                data['authors'], data['video_path'], data['duration']
            ))
    ...
    def update_item(self, item_id: int, item: MediaItem) -> bool:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                UPDATE media_items
                SET title = ?, status = ?, rating = ?, release_date = ?,
                    genres = ?, description = ?, authors = ?, video_path = ?,
                    duration = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            """, (
                data['title'], data['status'], data['rating'],
                data['release_date'], data['genres'], data['description'],
                data['authors'], data['video_path'], data['duration'],
                item_id
            ))
    ...
    def search_items(self, title: str = None, status: Status = None,
                     media_type: MediaType = None, min_rating: float = None,
                     genre: str = None, author: str = None) -> list[MediaItem]:
